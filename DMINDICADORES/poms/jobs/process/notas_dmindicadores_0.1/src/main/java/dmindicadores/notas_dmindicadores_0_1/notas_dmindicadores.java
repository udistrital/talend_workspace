...
package dmindicadores.notas_dmindicadores_0_1;
...
/**
 * Job: notas_dmindicadores Purpose: <br>
 * Description:  <br>
 * @author test@talend.com
 * @version 7.1.1.20180829_1603-M3
 * @status 
 */
public class notas_dmindicadores implements TalendJob {
	...
	private final String jobName = "notas_dmindicadores";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					notas_dmindicadores.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(notas_dmindicadores.this, new Object[] {
	...
	public static class admitidosStruct implements
		...
		final static byte[] commonByteArrayLock_DMINDICADORES_notas_dmindicadores = new byte[0];
		static byte[] commonByteArray_DMINDICADORES_notas_dmindicadores = new byte[0];

		public int NOT_CRA_COD;

		public int getNOT_CRA_COD() {
		...
		public int NOT_ASI_COD;

		public int getNOT_ASI_COD() {
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_DMINDICADORES_notas_dmindicadores.length) {
					if (length < 1024
							&& commonByteArray_DMINDICADORES_notas_dmindicadores.length == 0) {
						commonByteArray_DMINDICADORES_notas_dmindicadores = new byte[1024];
					} else {
						commonByteArray_DMINDICADORES_notas_dmindicadores = new byte[2 * length];
					}
				}
				dis.readFully(
						commonByteArray_DMINDICADORES_notas_dmindicadores, 0,
						length);
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_DMINDICADORES_notas_dmindicadores) {

				try {

					int length = 0;

					this.NOT_CRA_COD = dis.readInt();
		...
		public String toString() {
			...
			sb.append(",CURSADA=" + String.valueOf(CURSADA));
			sb.append(",OBS=" + OBS);
				...
				try {
					...
					while (rs_tPostgresqlInput_1.next()) {
						...
						if (dbschema_tPostgresqlOutput_1 == null
							...
							tableName_tPostgresqlOutput_1 = "notas";
						} else {
						...
						try (java.sql.ResultSet rsTable_tPostgresqlOutput_1 = dbMetaData_tPostgresqlOutput_1
							...
							while (rsTable_tPostgresqlOutput_1.next()) {
								...
								if (table_tPostgresqlOutput_1.equals("notas")
							...
							try (java.sql.Statement stmtCreate_tPostgresqlOutput_1 = conn_tPostgresqlOutput_1
								...
								stmtCreate_tPostgresqlOutput_1
										.execute("CREATE TABLE \""
												...
												+ "\"(\"NOT_CRA_COD\" INT4  not null ,\"PROYECTO\" VARCHAR(70)   not null ,\"NOT_EST_COD\" INT8  not null ,\"EST_NOMBRE\" VARCHAR(50)   not null ,\"NOT_ASI_COD\" INT4  not null ,\"ASI_NOMBRE\" VARCHAR(100)  ,\"NOT_SEM\" INT4 ,\"PERIODO\" VARCHAR(6)  ,\"NOTA\" FLOAT8 ,\"VALOR\" VARCHAR(50)   not null ,\"CURSADA\" INT4 ,\"OBS\" VARCHAR(50)  ,\"PROMEDIO\" FLOAT8 )");
							}
						}
						String insert_tPostgresqlOutput_1 = "INSERT INTO \""
								+ tableName_tPostgresqlOutput_1
								+ "\" (\"NOT_CRA_COD\",\"PROYECTO\",\"NOT_EST_COD\",\"EST_NOMBRE\",\"NOT_ASI_COD\",\"ASI_NOMBRE\",\"NOT_SEM\",\"PERIODO\",\"NOTA\",\"VALOR\",\"CURSADA\",\"OBS\",\"PROMEDIO\") VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
						...
						conn_tOracleInput_2 = java.sql.DriverManager
								.getConnection(url_tOracleInput_2,
						...
						String dbquery_tOracleInput_2 = "SELECT \nV.not_cra_cod,cra_nombre proyecto,\nV.not_est_cod,\nA.est_nombre,\nV.not_asi_cod,\nV.asi_nombre,\nV.not_sem,\nSUBSTR("
								+ "ultima,(LENGTH(V.not_asi_cod)+1),4)||'-'||SUBSTR(ultima,(LENGTH(V.not_asi_cod)+5),1) periodo,\nlpad(SUBSTR(ultima,(LENGTH"
								+ "(V.not_asi_cod)+6)),2,0) nota,\nM.mon_nombre VALOR,\nV.cursada,\nCASE\nWHEN N.not_obs=0 THEN ' '\nELSE O.nob_nombre\nEND obs,\n"
								+ "fa_promedio_nota(V.not_est_cod) promedio\nFROM v_acnot V\nINNER JOIN acest A ON V.not_est_cod=A.est_cod\nINNER JOIN accra C"
								+ " ON V.not_cra_cod=C.cra_cod\nINNER JOIN acasi S ON V.not_asi_cod=S.asi_cod\nINNER JOIN acnot N ON V.not_cra_cod=N.not_cra_"
								+ "cod AND V.not_est_cod=N.not_est_cod AND V.not_asi_cod=N.not_asi_cod AND SUBSTR(ultima,(LENGTH(V.not_asi_cod)+1),4)=N.not"
								+ "_ano AND SUBSTR(ultima,(LENGTH(V.not_asi_cod)+5),1)=N.not_per\nINNER JOIN acnotobs O ON N.not_obs=O.nob_cod\nINNER JOIN ac"
								+ "monto M ON SUBSTR(ultima,(LENGTH(V.not_asi_cod)+6))=M.mon_cod\nWHERE 1 = 1 AND decode(length(V.not_est_cod),7,(substr(V.n"
								+ "ot_est_cod,1,2)+1900),11,(substr(V.not_est_cod, 1,\n 8))) = "
								+ row3.ANNIO
								+ " ORDER BY V.not_sem, V.not_asi_cod \n\n";
						...
						try {
							...
							while (rs_tOracleInput_2.next()) {
								...
								if (colQtyInRs_tOracleInput_2 < 7) {
									...
									row1.NOT_SEM = rs_tOracleInput_2.getInt(7);

									if (row1.NOT_SEM == 0
											&& rs_tOracleInput_2.getObject(7) == null) {
								...
								if (colQtyInRs_tOracleInput_2 < 8) {
									...
									row1.PERIODO = routines.system.JDBCUtil
								...
								if (colQtyInRs_tOracleInput_2 < 9) {
									...
									row1.NOTA = routines.system.JDBCUtil
											.getDoubleObject2(
													rs_tOracleInput_2, 9);
								}
								if (colQtyInRs_tOracleInput_2 < 10) {
									...
									row1.VALOR = routines.system.JDBCUtil
											.getString(rs_tOracleInput_2, 10,
													true);
								}
								if (colQtyInRs_tOracleInput_2 < 11) {
									...
									row1.CURSADA = rs_tOracleInput_2.getInt(11);

									if (row1.CURSADA == 0
											&& rs_tOracleInput_2.getObject(11) == null) {
								...
								{ // start of Var scope
									...
									admitidos_tmp.NOT_CRA_COD = row1.NOT_CRA_COD;
									admitidos_tmp.PROYECTO = row1.PROYECTO;
									admitidos_tmp.NOT_EST_COD = row1.NOT_EST_COD;
									admitidos_tmp.EST_NOMBRE = row1.EST_NOMBRE;
									admitidos_tmp.NOT_ASI_COD = row1.NOT_ASI_COD;
									admitidos_tmp.ASI_NOMBRE = row1.ASI_NOMBRE;
									admitidos_tmp.NOT_SEM = row1.NOT_SEM;
									admitidos_tmp.PERIODO = row1.PERIODO;
									admitidos_tmp.NOTA = row1.NOTA;
									admitidos_tmp.VALOR = row1.VALOR;
									admitidos_tmp.CURSADA = row1.CURSADA;
									admitidos_tmp.OBS = row1.OBS;
								...
								if (admitidos != null) {
									...
									if (admitidos.PROMEDIO == null) {
										...
										pstmt_tPostgresqlOutput_1.setDouble(13,
												admitidos.PROMEDIO);
									}
									...
									if ((batchSize_tPostgresqlOutput_1 > 0)
										...
										try {
											...
											if (ne_tPostgresqlOutput_1 != null) {
												// build new exception to
												// provide the original cause
	...
	public static void main(String[] args) {
		final notas_dmindicadores notas_dmindicadoresClass = new notas_dmindicadores();

		int exitCode = notas_dmindicadoresClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = notas_dmindicadores.class
					.getClassLoader().getResourceAsStream(
							"dmindicadores/notas_dmindicadores_0_1/contexts/"
									+ contextStr + ".properties");
			if (inContext == null) {
				inContext = notas_dmindicadores.class
						.getClassLoader()
						.getResourceAsStream(
		...
		if (false) {
			System.out
					.println((endUsedMemory - startUsedMemory)
							+ " bytes memory increase when running : notas_dmindicadores");
		}
...
/************************************************************************************************
 * 177736 characters generated by Talend Open Studio for Data Integration on the
 * 12 de septiembre de 2018 04:40:34 PM COT
 ************************************************************************************************/

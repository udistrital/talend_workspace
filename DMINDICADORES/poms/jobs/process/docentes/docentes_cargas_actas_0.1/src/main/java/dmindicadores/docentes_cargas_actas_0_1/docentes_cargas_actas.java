...
package dmindicadores.docentes_cargas_actas_0_1;
...
/**
 * Job: docentes_cargas_actas Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 7.1.1.20180829_1603-M3
 * @status 
 */
public class docentes_cargas_actas implements TalendJob {
	...
	private final String jobName = "docentes_cargas_actas";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					docentes_cargas_actas.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(docentes_cargas_actas.this, new Object[] {
	...
	public static class pustos_asignadosStruct implements
			routines.system.IPersistableRow<pustos_asignadosStruct> {
		final static byte[] commonByteArrayLock_DMINDICADORES_docentes_cargas_actas = new byte[0];
		static byte[] commonByteArray_DMINDICADORES_docentes_cargas_actas = new byte[0];
		...
		public Double puntosSalariales;

		public Double getPuntosSalariales() {
			return this.puntosSalariales;
		}
		...
		public Double puntosBonificacion;

		public Double getPuntosBonificacion() {
			return this.puntosBonificacion;
		}
		...
		public java.util.Date fecha_actualizacion;
		...
		public java.util.Date F22;

		public java.util.Date getF22() {
			return this.F22;
		}
		...
		public int hashCode() {
			if (this.hashCodeDirty) {
				...
				result = prime * result + (int) this.ID_puntuacion;
		...
		public boolean equals(Object obj) {
			...
			if (getClass() != obj.getClass())
				return false;
			final pustos_asignadosStruct other = (pustos_asignadosStruct) obj;

			if (this.ID_puntuacion != other.ID_puntuacion)
				return false;

			return true;
		}

		public void copyDataTo(pustos_asignadosStruct other) {

			other.ID_puntuacion = this.ID_puntuacion;
			other.CC = this.CC;
			other.ID_item = this.ID_item;
			other.descripcion = this.descripcion;
			other.fecha = this.fecha;
			other.puntosSalariales = this.puntosSalariales;
			other.sesion = this.sesion;
			other.resolucion = this.resolucion;
			other.puntosBonificacion = this.puntosBonificacion;
			other.fecha_creacion = this.fecha_creacion;
			other.usuario_creacion = this.usuario_creacion;
			other.fecha_actualizacion = this.fecha_actualizacion;
			other.usuario_actualizacion = this.usuario_actualizacion;
			other.F14 = this.F14;
			other.F15 = this.F15;
			other.F16 = this.F16;
			other.F17 = this.F17;
			...
			other.F23 = this.F23;
			other.F24 = this.F24;
			other.Campo0 = this.Campo0;

		}

		public void copyKeysDataTo(pustos_asignadosStruct other) {
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_DMINDICADORES_docentes_cargas_actas.length) {
					if (length < 1024
							&& commonByteArray_DMINDICADORES_docentes_cargas_actas.length == 0) {
						commonByteArray_DMINDICADORES_docentes_cargas_actas = new byte[1024];
					} else {
						commonByteArray_DMINDICADORES_docentes_cargas_actas = new byte[2 * length];
					}
				}
				dis.readFully(
						commonByteArray_DMINDICADORES_docentes_cargas_actas, 0,
						length);
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_DMINDICADORES_docentes_cargas_actas) {

				try {

					int length = 0;

					this.ID_puntuacion = dis.readInt();

					this.CC = readInteger(dis);

					this.ID_item = readInteger(dis);
					...
					length = dis.readByte();
					if (length == -1) {
						this.puntosSalariales = null;
					} else {
						this.puntosSalariales = dis.readDouble();
					}

					this.sesion = readInteger(dis);

					this.resolucion = readString(dis);

					length = dis.readByte();
					if (length == -1) {
						this.puntosBonificacion = null;
					} else {
						this.puntosBonificacion = dis.readDouble();
					}

					this.fecha_creacion = readDate(dis);

					this.usuario_creacion = readString(dis);

					this.fecha_actualizacion = readDate(dis);

					this.usuario_actualizacion = readString(dis);

					length = dis.readByte();
					if (length == -1) {
						this.F14 = null;
					} else {
						this.F14 = dis.readBoolean();
					}
					...
					if (length == -1) {
						...
						this.F15 = dis.readDouble();
					}

					this.F16 = readString(dis);

					this.F17 = readString(dis);

					this.F18 = readString(dis);

					this.F19 = readString(dis);

					this.F20 = readString(dis);

					this.F21 = readString(dis);

					this.F22 = readDate(dis);

					this.F23 = readString(dis);

					this.F24 = readString(dis);

					this.Campo0 = readString(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				dos.writeInt(this.ID_puntuacion);

				// Integer

				writeInteger(this.CC, dos);

				// Integer

				writeInteger(this.ID_item, dos);
				...
				if (this.puntosSalariales == null) {
					dos.writeByte(-1);
				} else {
					dos.writeByte(0);
					dos.writeDouble(this.puntosSalariales);
				}

				// Integer

				writeInteger(this.sesion, dos);
				...
				if (this.puntosBonificacion == null) {
					dos.writeByte(-1);
				} else {
					dos.writeByte(0);
					dos.writeDouble(this.puntosBonificacion);
				}
				...
				writeString(this.usuario_creacion, dos);

				// java.util.Date

				writeDate(this.fecha_actualizacion, dos);

				// String

				writeString(this.usuario_actualizacion, dos);
				...
				if (this.F15 == null) {
					...
					dos.writeByte(0);
					dos.writeDouble(this.F15);
				}

				// String

				writeString(this.F16, dos);

				// String

				writeString(this.F17, dos);

				// String

				writeString(this.F18, dos);

				// String

				writeString(this.F19, dos);

				// String

				writeString(this.F20, dos);

				// String

				writeString(this.F21, dos);

				// java.util.Date

				writeDate(this.F22, dos);

				// String

				writeString(this.F23, dos);

				// String

				writeString(this.F24, dos);

				// String

				writeString(this.Campo0, dos);
		...
		public String toString() {
			...
			sb.append("[");
			sb.append("ID_puntuacion=" + String.valueOf(ID_puntuacion));
			sb.append(",CC=" + String.valueOf(CC));
			sb.append(",ID_item=" + String.valueOf(ID_item));
			sb.append(",descripcion=" + descripcion);
			sb.append(",fecha=" + String.valueOf(fecha));
			sb.append(",puntosSalariales=" + String.valueOf(puntosSalariales));
			sb.append(",sesion=" + String.valueOf(sesion));
			sb.append(",resolucion=" + resolucion);
			sb.append(",puntosBonificacion="
					+ String.valueOf(puntosBonificacion));
			sb.append(",fecha_creacion=" + String.valueOf(fecha_creacion));
			sb.append(",usuario_creacion=" + usuario_creacion);
			sb.append(",fecha_actualizacion="
					+ String.valueOf(fecha_actualizacion));
			sb.append(",usuario_actualizacion=" + usuario_actualizacion);
			sb.append(",F14=" + String.valueOf(F14));
			sb.append(",F15=" + String.valueOf(F15));
			sb.append(",F16=" + F16);
			sb.append(",F17=" + F17);
			sb.append(",F18=" + F18);
			sb.append(",F19=" + F19);
			sb.append(",F20=" + F20);
			sb.append(",F21=" + F21);
			sb.append(",F22=" + String.valueOf(F22));
			sb.append(",F23=" + F23);
			sb.append(",F24=" + F24);
			sb.append(",Campo0=" + Campo0);
			sb.append("]");
		 ...
		 * Compare keys
		 */
		public int compareTo(pustos_asignadosStruct other) {

			int returnValue = -1;

			returnValue = checkNullsAndCompare(this.ID_puntuacion,
					other.ID_puntuacion);
			if (returnValue != 0) {
				return returnValue;
			}
	...
	public static class row1Struct implements
		...
		public Integer CODIGO_DOCENTE;

		public Integer getCODIGO_DOCENTE() {
		...
		public String getFECHA_ACTA() {
			return this.FECHA_ACTA;
		}

		public String RECTORIA;

		public String getRECTORIA() {
			return this.RECTORIA;
		}

		public String PUNTOS_SALARIALES;

		public String getPUNTOS_SALARIALES() {
			return this.PUNTOS_SALARIALES;
		}

		public String PUNTOS_BONIFICACION;

		public String getPUNTOS_BONIFICACION() {
			return this.PUNTOS_BONIFICACION;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_DMINDICADORES_docentes_cargas_actas) {
				...
				try {
					...
					this.CODIGO_DOCENTE = readInteger(dis);

					this.CEDULA = readInteger(dis);

					this.DOCENTE = readString(dis);

					this.FACULTAD = readString(dis);

					this.ID_CONCEPTO = readInteger(dis);

					this.CONCEPTO = readString(dis);

					this.SESION = readInteger(dis);

					this.FECHA_ACTA = readString(dis);

					this.RECTORIA = readString(dis);

					this.PUNTOS_SALARIALES = readString(dis);

					this.PUNTOS_BONIFICACION = readString(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeInteger(this.CODIGO_DOCENTE, dos);

				// Integer

				writeInteger(this.CEDULA, dos);
				...
				writeInteger(this.ID_CONCEPTO, dos);

				// String

				writeString(this.CONCEPTO, dos);
				...
				writeString(this.FECHA_ACTA, dos);

				// String

				writeString(this.RECTORIA, dos);

				// String

				writeString(this.PUNTOS_SALARIALES, dos);

				// String

				writeString(this.PUNTOS_BONIFICACION, dos);
		...
		public String toString() {
			...
			sb.append(",CEDULA=" + String.valueOf(CEDULA));
			sb.append(",DOCENTE=" + DOCENTE);
			sb.append(",FACULTAD=" + FACULTAD);
			sb.append(",ID_CONCEPTO=" + String.valueOf(ID_CONCEPTO));
			sb.append(",CONCEPTO=" + CONCEPTO);
			sb.append(",SESION=" + String.valueOf(SESION));
			sb.append(",FECHA_ACTA=" + FECHA_ACTA);
			sb.append(",RECTORIA=" + RECTORIA);
			sb.append(",PUNTOS_SALARIALES=" + PUNTOS_SALARIALES);
			sb.append(",PUNTOS_BONIFICACION=" + PUNTOS_BONIFICACION);
			sb.append(",RESOLUCION=" + RESOLUCION);
			sb.append("]");
	...
	public void tPostgresqlInput_1Process(
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				pustos_asignadosStruct pustos_asignados = new pustos_asignadosStruct();
				...
				if (execStat) {
					if (resourceMap.get("inIterateVComp") == null) {
						...
						runStat.updateStatOnConnection("pustos_asignados"
								+ iterateId, 0, 0);
				...
				String tableName_tMysqlOutput_1 = "tblAsignadosSesionesPuntos";
				...
				if (dbProperties_tMysqlOutput_1 == null
					...
					url_tMysqlOutput_1 = "jdbc:mysql://" + "10.20.0.88" + ":"
							+ "3306" + "/" + "docencia" + "?"
				...
				String dbUser_tMysqlOutput_1 = "docencia";
				...
				if (!whetherExist_tMysqlOutput_1) {
					try (java.sql.Statement stmtCreate_tMysqlOutput_1 = conn_tMysqlOutput_1
							.createStatement()) {
						stmtCreate_tMysqlOutput_1
								.execute("CREATE TABLE `"
										+ tableName_tMysqlOutput_1
										+ "`(`ID_puntuacion` INT(10)   not null ,`CC` INT(10)  ,`ID_item` INT(10)  ,`descripcion` VARCHAR(16777215)  ,`fecha` DATETIME ,`puntosSalariales` DOUBLE(22,0)  ,`sesion` INT(10)  ,`resolucion` VARCHAR(50)  ,`puntosBonificación` DOUBLE(22,0)  ,`fecha_creacion` DATETIME ,`usuario_creacion` VARCHAR(255)  ,`fecha_actualizacion` DATETIME ,`usuario_actualizacion` VARCHAR(255)  ,`F14` BIT(1)  default 'b'0' ,`F15` DOUBLE(22,0)  ,`F16` VARCHAR(255)  ,`F17` VARCHAR(255)  ,`F18` VARCHAR(255)  ,`F19` VARCHAR(255)  ,`F20` VARCHAR(255)  ,`F21` VARCHAR(255)  ,`F22` DATETIME ,`F23` VARCHAR(255)  ,`F24` VARCHAR(255)  ,`Campo0` VARCHAR(16777215)  ,primary key(`ID_puntuacion`))");
					}
				}

				String insert_tMysqlOutput_1 = "INSERT INTO `"
						+ "tblAsignadosSesionesPuntos"
						+ "` (`ID_puntuacion`,`CC`,`ID_item`,`descripcion`,`fecha`,`puntosSalariales`,`sesion`,`resolucion`,`puntosBonificación`,`fecha_creacion`,`usuario_creacion`,`fecha_actualizacion`,`usuario_actualizacion`,`F14`,`F15`,`F16`,`F17`,`F18`,`F19`,`F20`,`F21`,`F22`,`F23`,`F24`,`Campo0`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
				...
				pustos_asignadosStruct pustos_asignados_tmp = new pustos_asignadosStruct();
				// ###############################
					...
					while (rs_tPostgresqlInput_1.next()) {
						...
						if (colQtyInRs_tPostgresqlInput_1 < 1) {
							...
							if (rs_tPostgresqlInput_1.getObject(1) != null) {
								row1.CODIGO_DOCENTE = rs_tPostgresqlInput_1
										.getInt(1);
							} else {
								row1.CODIGO_DOCENTE = null;
							}
						...
						if (colQtyInRs_tPostgresqlInput_1 < 2) {
							...
							if (rs_tPostgresqlInput_1.getObject(2) != null) {
								row1.CEDULA = rs_tPostgresqlInput_1.getInt(2);
							} else {
								row1.CEDULA = null;
							}
						}
						...
						if (colQtyInRs_tPostgresqlInput_1 < 4) {
							...
							row1.FACULTAD = routines.system.JDBCUtil.getString(
									rs_tPostgresqlInput_1, 4, false);
						}
						if (colQtyInRs_tPostgresqlInput_1 < 5) {
							row1.ID_CONCEPTO = null;
						} else {

							if (rs_tPostgresqlInput_1.getObject(5) != null) {
								row1.ID_CONCEPTO = rs_tPostgresqlInput_1
										.getInt(5);
							} else {
								row1.ID_CONCEPTO = null;
							}
						...
						if (colQtyInRs_tPostgresqlInput_1 < 6) {
							...
							row1.CONCEPTO = routines.system.JDBCUtil.getString(
									rs_tPostgresqlInput_1, 6, false);
						}
						if (colQtyInRs_tPostgresqlInput_1 < 7) {
							...
							if (rs_tPostgresqlInput_1.getObject(7) != null) {
								row1.SESION = rs_tPostgresqlInput_1.getInt(7);
							} else {
								row1.SESION = null;
							}
						}
						if (colQtyInRs_tPostgresqlInput_1 < 8) {
							row1.FECHA_ACTA = null;
						} else {

							row1.FECHA_ACTA = routines.system.JDBCUtil
						...
						if (colQtyInRs_tPostgresqlInput_1 < 9) {
							...
							row1.RECTORIA = routines.system.JDBCUtil.getString(
									rs_tPostgresqlInput_1, 9, false);
						}
						if (colQtyInRs_tPostgresqlInput_1 < 10) {
							row1.PUNTOS_SALARIALES = null;
						} else {

							row1.PUNTOS_SALARIALES = routines.system.JDBCUtil
									.getString(rs_tPostgresqlInput_1, 10, false);
						}
						if (colQtyInRs_tPostgresqlInput_1 < 11) {
							row1.PUNTOS_BONIFICACION = null;
						} else {

							row1.PUNTOS_BONIFICACION = routines.system.JDBCUtil
									.getString(rs_tPostgresqlInput_1, 11, false);
						}
						if (colQtyInRs_tPostgresqlInput_1 < 12) {
							row1.RESOLUCION = null;
						} else {

							row1.RESOLUCION = routines.system.JDBCUtil
						...
						{ // start of Var scope
							...
							pustos_asignados = null;

							// # Output table : 'pustos_asignados'
							pustos_asignados_tmp.ID_puntuacion = 0;
							pustos_asignados_tmp.CC = row1.CEDULA;
							pustos_asignados_tmp.ID_item = row1.ID_CONCEPTO;
							pustos_asignados_tmp.descripcion = row1.CONCEPTO;
							pustos_asignados_tmp.fecha = TalendDate.parseDate(
									"dd/MM/yyyy", row1.FECHA_ACTA);
							;
							pustos_asignados_tmp.puntosSalariales = null;
							pustos_asignados_tmp.sesion = row1.SESION;
							pustos_asignados_tmp.resolucion = row1.RESOLUCION;
							pustos_asignados_tmp.puntosBonificacion = row1.PUNTOS_BONIFICACION == null ? null
									: row1.PUNTOS_BONIFICACION == "" ? null
											: Double.parseDouble(row1.PUNTOS_BONIFICACION
													.replace(',', '.'));
							pustos_asignados_tmp.fecha_creacion = TalendDate
									.parseDate("dd-MM-yyyy",
											TalendDate.getDate("dd-MM-yyyy"));
							pustos_asignados_tmp.usuario_creacion = "OAS_talend";
							pustos_asignados_tmp.fecha_actualizacion = null;
							pustos_asignados_tmp.usuario_actualizacion = null;
							pustos_asignados_tmp.F14 = null;
							pustos_asignados_tmp.F15 = null;
							pustos_asignados_tmp.F16 = null;
							pustos_asignados_tmp.F17 = null;
							pustos_asignados_tmp.F18 = null;
							pustos_asignados_tmp.F19 = null;
							pustos_asignados_tmp.F20 = null;
							pustos_asignados_tmp.F21 = null;
							pustos_asignados_tmp.F22 = null;
							pustos_asignados_tmp.F23 = null;
							pustos_asignados_tmp.F24 = null;
							pustos_asignados_tmp.Campo0 = null;
							pustos_asignados = pustos_asignados_tmp;
							// ###############################
						 ...
						 */
						// Start of branch "pustos_asignados"
						if (pustos_asignados != null) {
							...
							if (execStat) {
								runStat.updateStatOnConnection(
										"pustos_asignados" + iterateId, 1, 1);
							}

							whetherReject_tMysqlOutput_1 = false;
							pstmt_tMysqlOutput_1.setInt(1,
									pustos_asignados.ID_puntuacion);

							if (pustos_asignados.CC == null) {
							...
							if (pustos_asignados.ID_item == null) {
								pstmt_tMysqlOutput_1.setNull(3,
										java.sql.Types.INTEGER);
							} else {
								pstmt_tMysqlOutput_1.setInt(3,
										pustos_asignados.ID_item);
							}

							if (pustos_asignados.descripcion == null) {
								pstmt_tMysqlOutput_1.setNull(4,
										java.sql.Types.VARCHAR);
							} else {
								pstmt_tMysqlOutput_1.setString(4,
										pustos_asignados.descripcion);
							}

							if (pustos_asignados.fecha != null) {
								date_tMysqlOutput_1 = pustos_asignados.fecha
										.getTime();
							...
							if (pustos_asignados.puntosSalariales == null) {
								pstmt_tMysqlOutput_1.setNull(6,
										java.sql.Types.DOUBLE);
							} else {
								pstmt_tMysqlOutput_1.setDouble(6,
										pustos_asignados.puntosSalariales);
							}

							if (pustos_asignados.sesion == null) {
								pstmt_tMysqlOutput_1.setNull(7,
										java.sql.Types.INTEGER);
							} else {
								pstmt_tMysqlOutput_1.setInt(7,
										pustos_asignados.sesion);
							}

							if (pustos_asignados.resolucion == null) {
								pstmt_tMysqlOutput_1.setNull(8,
										java.sql.Types.VARCHAR);
							} else {
								pstmt_tMysqlOutput_1.setString(8,
										pustos_asignados.resolucion);
							}

							if (pustos_asignados.puntosBonificacion == null) {
								pstmt_tMysqlOutput_1.setNull(9,
										java.sql.Types.DOUBLE);
							} else {
								pstmt_tMysqlOutput_1.setDouble(9,
										pustos_asignados.puntosBonificacion);
							}

							if (pustos_asignados.fecha_creacion != null) {
								date_tMysqlOutput_1 = pustos_asignados.fecha_creacion
										.getTime();
								if (date_tMysqlOutput_1 < year1_tMysqlOutput_1
										|| date_tMysqlOutput_1 >= year10000_tMysqlOutput_1) {
							...
							if (pustos_asignados.usuario_creacion == null) {
								pstmt_tMysqlOutput_1.setNull(11,
										java.sql.Types.VARCHAR);
							} else {
								pstmt_tMysqlOutput_1.setString(11,
										pustos_asignados.usuario_creacion);
							}

							if (pustos_asignados.fecha_actualizacion != null) {
								date_tMysqlOutput_1 = pustos_asignados.fecha_actualizacion
										.getTime();
								if (date_tMysqlOutput_1 < year1_tMysqlOutput_1
										|| date_tMysqlOutput_1 >= year10000_tMysqlOutput_1) {
							...
							if (pustos_asignados.usuario_actualizacion == null) {
								pstmt_tMysqlOutput_1.setNull(13,
										java.sql.Types.VARCHAR);
							} else {
								pstmt_tMysqlOutput_1.setString(13,
										pustos_asignados.usuario_actualizacion);
							}

							if (pustos_asignados.F14 == null) {
								pstmt_tMysqlOutput_1.setNull(14,
										java.sql.Types.BOOLEAN);
							} else {
								pstmt_tMysqlOutput_1.setBoolean(14,
										pustos_asignados.F14);
							}

							if (pustos_asignados.F15 == null) {
								pstmt_tMysqlOutput_1.setNull(15,
										java.sql.Types.DOUBLE);
							} else {
								pstmt_tMysqlOutput_1.setDouble(15,
										pustos_asignados.F15);
							}

							if (pustos_asignados.F16 == null) {
								pstmt_tMysqlOutput_1.setNull(16,
										java.sql.Types.VARCHAR);
							} else {
								pstmt_tMysqlOutput_1.setString(16,
										pustos_asignados.F16);
							}

							if (pustos_asignados.F17 == null) {
								pstmt_tMysqlOutput_1.setNull(17,
										java.sql.Types.VARCHAR);
							} else {
								pstmt_tMysqlOutput_1.setString(17,
										pustos_asignados.F17);
							}

							if (pustos_asignados.F18 == null) {
								pstmt_tMysqlOutput_1.setNull(18,
										java.sql.Types.VARCHAR);
							} else {
								pstmt_tMysqlOutput_1.setString(18,
										pustos_asignados.F18);
							}

							if (pustos_asignados.F19 == null) {
								pstmt_tMysqlOutput_1.setNull(19,
										java.sql.Types.VARCHAR);
							} else {
								pstmt_tMysqlOutput_1.setString(19,
										pustos_asignados.F19);
							}

							if (pustos_asignados.F20 == null) {
								pstmt_tMysqlOutput_1.setNull(20,
										java.sql.Types.VARCHAR);
							} else {
								pstmt_tMysqlOutput_1.setString(20,
										pustos_asignados.F20);
							}

							if (pustos_asignados.F21 == null) {
								pstmt_tMysqlOutput_1.setNull(21,
										java.sql.Types.VARCHAR);
							} else {
								pstmt_tMysqlOutput_1.setString(21,
										pustos_asignados.F21);
							}

							if (pustos_asignados.F22 != null) {
								date_tMysqlOutput_1 = pustos_asignados.F22
										.getTime();
								if (date_tMysqlOutput_1 < year1_tMysqlOutput_1
										|| date_tMysqlOutput_1 >= year10000_tMysqlOutput_1) {
						...
						} // End of branch "pustos_asignados"
	...
	public static void main(String[] args) {
		final docentes_cargas_actas docentes_cargas_actasClass = new docentes_cargas_actas();

		int exitCode = docentes_cargas_actasClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = docentes_cargas_actas.class
					.getClassLoader().getResourceAsStream(
							"dmindicadores/docentes_cargas_actas_0_1/contexts/"
									+ contextStr + ".properties");
			if (inContext == null) {
				inContext = docentes_cargas_actas.class
						.getClassLoader()
						.getResourceAsStream(
		...
		if (false) {
			System.out
					.println((endUsedMemory - startUsedMemory)
							+ " bytes memory increase when running : docentes_cargas_actas");
		}
...
/************************************************************************************************
 * 167982 characters generated by Talend Open Studio for Data Integration on the
 * 5 de septiembre de 2018 02:32:42 PM COT
 ************************************************************************************************/

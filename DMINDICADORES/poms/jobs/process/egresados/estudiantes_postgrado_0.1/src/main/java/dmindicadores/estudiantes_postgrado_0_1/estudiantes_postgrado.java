...
package dmindicadores.estudiantes_postgrado_0_1;
...
/**
 * Job: estudiantes_postgrado Purpose: <br>
 * Description:  <br>
 * @author user@talend.com
 * @version 7.1.1.20180829_1603-M3
 * @status 
 */
public class estudiantes_postgrado implements TalendJob {
	...
	private final String jobName = "estudiantes_postgrado";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					estudiantes_postgrado.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(estudiantes_postgrado.this, new Object[] {
	...
	public static class graduados_proyectosStruct implements
			routines.system.IPersistableRow<graduados_proyectosStruct> {
		final static byte[] commonByteArrayLock_DMINDICADORES_estudiantes_postgrado = new byte[0];
		static byte[] commonByteArray_DMINDICADORES_estudiantes_postgrado = new byte[0];
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_DMINDICADORES_estudiantes_postgrado) {
		 ...
		 * Compare keys
		 */
		public int compareTo(graduados_proyectosStruct other) {
	...
	public void tPostgresqlInput_1Process(
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				graduados_proyectosStruct graduados_proyectos = new graduados_proyectosStruct();
				...
				if (execStat) {
					if (resourceMap.get("inIterateVComp") == null) {
						...
						runStat.updateStatOnConnection("graduados_proyectos"
								+ iterateId, 0, 0);
				...
				if (dbschema_tPostgresqlOutput_1 == null
					...
					tableName_tPostgresqlOutput_1 = "graduados_proyectos";
				} else {
					...
					try (java.sql.Statement stmtCreate_tPostgresqlOutput_1 = conn_tPostgresqlOutput_1
						...
						stmtCreate_tPostgresqlOutput_1
								.execute("CREATE TABLE \""
										...
										+ "\"(\"count\" INT8 ,\"identificacion\" INT8 )");
					}
				}
				String insert_tPostgresqlOutput_1 = "INSERT INTO \""
						+ tableName_tPostgresqlOutput_1
						+ "\" (\"count\",\"identificacion\") VALUES (?,?)";
				...
				graduados_proyectosStruct graduados_proyectos_tmp = new graduados_proyectosStruct();
				// ###############################
				...
				String dbquery_tPostgresqlInput_1 = "\n       SELECT  count(codigo_proyecto), identificacion FROM estudiantes \n	where codigo_estado in ('A','J','B','E')\n	gro"
						+ "up by identificacion\n       order by identificacion";
					...
					while (rs_tPostgresqlInput_1.next()) {
						...
						if (colQtyInRs_tPostgresqlInput_1 < 1) {
							...
							if (rs_tPostgresqlInput_1.getObject(1) != null) {
								row1.count = rs_tPostgresqlInput_1.getLong(1);
							} else {
								row1.count = null;
							}
						}
						if (colQtyInRs_tPostgresqlInput_1 < 2) {
							...
							if (rs_tPostgresqlInput_1.getObject(2) != null) {
								row1.identificacion = rs_tPostgresqlInput_1
										.getLong(2);
							} else {
								row1.identificacion = null;
							}
						...
						{ // start of Var scope
							...
							graduados_proyectos = null;

							// # Output table : 'graduados_proyectos'
							graduados_proyectos_tmp.count = row1.count;
							graduados_proyectos_tmp.identificacion = row1.identificacion;
							graduados_proyectos = graduados_proyectos_tmp;
							// ###############################
						 ...
						 */
						// Start of branch "graduados_proyectos"
						if (graduados_proyectos != null) {
							...
							if (execStat) {
								runStat.updateStatOnConnection(
										"graduados_proyectos" + iterateId, 1, 1);
							}

							whetherReject_tPostgresqlOutput_1 = false;
							if (graduados_proyectos.count == null) {
								pstmt_tPostgresqlOutput_1.setNull(1,
										java.sql.Types.INTEGER);
							} else {
								pstmt_tPostgresqlOutput_1.setLong(1,
										graduados_proyectos.count);
							}

							if (graduados_proyectos.identificacion == null) {
								pstmt_tPostgresqlOutput_1.setNull(2,
										java.sql.Types.INTEGER);
							} else {
								pstmt_tPostgresqlOutput_1.setLong(2,
										graduados_proyectos.identificacion);
							}
						...
						} // End of branch "graduados_proyectos"
	...
	public static class row_talendStats_STATSStruct implements
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_DMINDICADORES_estudiantes_postgrado.length) {
					if (length < 1024
							&& commonByteArray_DMINDICADORES_estudiantes_postgrado.length == 0) {
						commonByteArray_DMINDICADORES_estudiantes_postgrado = new byte[1024];
					} else {
						commonByteArray_DMINDICADORES_estudiantes_postgrado = new byte[2 * length];
					}
				}
				dis.readFully(
						commonByteArray_DMINDICADORES_estudiantes_postgrado, 0,
						length);
	...
	public static void main(String[] args) {
		final estudiantes_postgrado estudiantes_postgradoClass = new estudiantes_postgrado();

		int exitCode = estudiantes_postgradoClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = estudiantes_postgrado.class
					.getClassLoader().getResourceAsStream(
							"dmindicadores/estudiantes_postgrado_0_1/contexts/"
									+ contextStr + ".properties");
			if (inContext == null) {
				inContext = estudiantes_postgrado.class
						.getClassLoader()
						.getResourceAsStream(
		...
		if (false) {
			System.out
					.println((endUsedMemory - startUsedMemory)
							+ " bytes memory increase when running : estudiantes_postgrado");
		}
...
/************************************************************************************************
 * 147964 characters generated by Talend Open Studio for Data Integration on the
 * 19 de septiembre de 2018 09:45:47 AM COT
 ************************************************************************************************/

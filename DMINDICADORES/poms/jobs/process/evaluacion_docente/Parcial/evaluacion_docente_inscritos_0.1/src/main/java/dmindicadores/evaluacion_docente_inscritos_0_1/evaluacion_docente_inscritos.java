...
package dmindicadores.evaluacion_docente_inscritos_0_1;
...
/**
 * Job: evaluacion_docente_inscritos Purpose: <br>
 * Description:  <br>
 * @author 
 * @version 7.1.1.20180829_1603-M3
 * @status 
 */
public class evaluacion_docente_inscritos implements TalendJob {
	...
	private final String jobName = "evaluacion_docente_inscritos";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					evaluacion_docente_inscritos.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(evaluacion_docente_inscritos.this,
	...
	public static class InscritosStruct implements
		...
		final static byte[] commonByteArrayLock_DMINDICADORES_evaluacion_docente_inscritos = new byte[0];
		static byte[] commonByteArray_DMINDICADORES_evaluacion_docente_inscritos = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_DMINDICADORES_evaluacion_docente_inscritos.length) {
					if (length < 1024
							&& commonByteArray_DMINDICADORES_evaluacion_docente_inscritos.length == 0) {
						commonByteArray_DMINDICADORES_evaluacion_docente_inscritos = new byte[1024];
					} else {
						commonByteArray_DMINDICADORES_evaluacion_docente_inscritos = new byte[2 * length];
					}
				}
				dis.readFully(
						commonByteArray_DMINDICADORES_evaluacion_docente_inscritos,
						0, length);
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_DMINDICADORES_evaluacion_docente_inscritos) {
		...
		public String toString() {
			...
			sb.append(",CODIGO_TIPO_VINCULACION="
					+ String.valueOf(CODIGO_TIPO_VINCULACION));
	...
	public void tPostgresqlInput_1Process(
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				String dbquery_tPostgresqlInput_1 = "SELECT id_anno, id_semestre\n  FROM \"CO_UDISTRITAL_MDM\".periodo where activo=true";
					...
					while (rs_tPostgresqlInput_1.next()) {
						...
						if (colQtyInRs_tPostgresqlInput_1 < 1) {
							...
							if (rs_tPostgresqlInput_1.getObject(1) != null) {
								row5.id_anno = rs_tPostgresqlInput_1.getInt(1);
							} else {
						...
						if (colQtyInRs_tPostgresqlInput_1 < 2) {
							...
							if (rs_tPostgresqlInput_1.getObject(2) != null) {
								row5.id_semestre = rs_tPostgresqlInput_1
										.getInt(2);
							} else {
								throw new RuntimeException(
										"Null value in non-Nullable column");
						...
						query_tPostgresqlRow_1 = "DELETE FROM evaluacion_docente_inscritos\n WHERE \"ANNO\"=2018 and \"SEMESTRE\"=1";
						...
						if (dbschema_tPostgresqlOutput_1 == null
							...
							tableName_tPostgresqlOutput_1 = "evaluacion_docente_inscritos";
						} else {
							tableName_tPostgresqlOutput_1 = dbschema_tPostgresqlOutput_1
									+ "\".\"" + "evaluacion_docente_inscritos";
						}
						...
						try (java.sql.ResultSet rsTable_tPostgresqlOutput_1 = dbMetaData_tPostgresqlOutput_1
							...
							while (rsTable_tPostgresqlOutput_1.next()) {
								...
								if (table_tPostgresqlOutput_1
										.equals("evaluacion_docente_inscritos")
							...
							try (java.sql.Statement stmtCreate_tPostgresqlOutput_1 = conn_tPostgresqlOutput_1
								...
								stmtCreate_tPostgresqlOutput_1
										.execute("CREATE TABLE \""
												...
												+ "\"(\"ANNO\" INT4  not null ,\"SEMESTRE\" INT4  not null ,\"CODIGO_FACULTAD\" INT4  not null ,\"FACULTAD\" VARCHAR(100)   not null ,\"CODIGO_PROYECTO\" INT4  not null ,\"PROYECTO\" VARCHAR(100)   not null ,\"CODIGO_ESPACIO\" INT4  not null ,\"ESPACIO\" VARCHAR(100)  ,\"ESTUDIANTE\" INT8  not null ,\"GRUPO\" VARCHAR(44)  ,\"CODIGO_DOCENTE\" INT8  not null ,\"DOCENTE\" VARCHAR(52)  ,\"CODIGO_PROYECTO_ESTUDIANTE\" INT4 ,\"PROYECTO_ESTUDIANTE\" VARCHAR ,\"CODIGO_FACULTAD_ESTUDIANTE\" INT4 ,\"FACULTAD_ESTUDIANTE\" VARCHAR ,\"CODIGO_TIPO_VINCULACION\" INT4 ,\"VINCULACION\" VARCHAR(45)  ,\"FECHA_CARGA\" TIMESTAMP(0)  )");
							}
						}
						String insert_tPostgresqlOutput_1 = "INSERT INTO \""
								+ tableName_tPostgresqlOutput_1
								+ "\" (\"ANNO\",\"SEMESTRE\",\"CODIGO_FACULTAD\",\"FACULTAD\",\"CODIGO_PROYECTO\",\"PROYECTO\",\"CODIGO_ESPACIO\",\"ESPACIO\",\"ESTUDIANTE\",\"GRUPO\",\"CODIGO_DOCENTE\",\"DOCENTE\",\"CODIGO_PROYECTO_ESTUDIANTE\",\"PROYECTO_ESTUDIANTE\",\"CODIGO_FACULTAD_ESTUDIANTE\",\"FACULTAD_ESTUDIANTE\",\"CODIGO_TIPO_VINCULACION\",\"VINCULACION\",\"FECHA_CARGA\") VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
						...
						String dbquery_tOracleInput_1 = "SELECT distinct\n  ins_ano        AS anno,\n    ins_per        AS semestre,\n    dep_cod     AS codigo_facultad,\n    dep_n"
								...
								+ "te,\n car_tip_vin as codigo_tipo_vinculacion,\n tvi_nombre as vinculacion,\ncurrent_date AS fecha_carga\nFROM acins\nINNER JO"
								+ "IN acasi ON ins_asi_cod=asi_cod\nINNER JOIN acasperi ON ins_ano=acasperi.ape_ano AND  ins_per=ape_per \nINNER JOIN accurso"
								+ "s ON ins_gr=cur_id\nINNER JOIN accra ON cur_cra_cod=cra_cod\nINNER JOIN gedep ON cra_dep_cod=dep_cod\nINNER JOIN achorarios"
								+ " ON hor_id_curso=cur_id\nINNER JOIN accargas  ON car_hor_id=hor_id\nINNER JOIN acdocente ON car_doc_nro = doc_nro_iden\nINN"
								+ "ER JOIN acest ON est_cod=ins_est_cod\nINNER JOIN actipvin on tvi_cod=car_tip_vin\nWHERE ins_ano= 2018 and ins_per= 3\nORDER"
								+ " BY dep_cod,cra_cod,ins_asi_cod";
							...
							while (rs_tOracleInput_1.next()) {
								...
								if (colQtyInRs_tOracleInput_1 < 2) {
									...
									if (rs_tOracleInput_1.getObject(2) != null) {
										row1.SEMESTRE = rs_tOracleInput_1
												.getInt(2);
									} else {
								...
								if (colQtyInRs_tOracleInput_1 < 8) {
									...
									row1.ESPACIO = routines.system.JDBCUtil
								...
								if (colQtyInRs_tOracleInput_1 < 9) {
									...
									if (rs_tOracleInput_1.getObject(9) != null) {
										row1.ESTUDIANTE = rs_tOracleInput_1
												.getLong(9);
									} else {
								...
								if (colQtyInRs_tOracleInput_1 < 12) {
									...
									row1.DOCENTE = routines.system.JDBCUtil
											.getString(rs_tOracleInput_1, 12,
													false);
								}
								...
								if (colQtyInRs_tOracleInput_1 < 14) {
									...
									if (rs_tOracleInput_1.getObject(14) != null) {
										row1.CODIGO_TIPO_VINCULACION = rs_tOracleInput_1
												.getInt(14);
									} else {
								...
								if (tHash_Lookup_row2 != null
									...
									// System.out.println("WARNING: UNIQUE MATCH is configured for the lookup 'row2' and it contains more one result from keys :  row2.CRA_COD = '"
									// + row2HashKey.CRA_COD + "'");
								} // G 071
								...
								{ // start of Var scope
									...
									Inscritos_tmp.PROYECTO = TalendString
											.removeAccents(row1.PROYECTO);
									...
									Inscritos_tmp.PROYECTO_ESTUDIANTE = TalendString
											.removeAccents(row2.CRA_NOMBRE);
									Inscritos_tmp.CODIGO_FACULTAD_ESTUDIANTE = row2.DEP_COD;
									Inscritos_tmp.FACULTAD_ESTUDIANTE = row2.DEP_NOMBRE;
									Inscritos_tmp.CODIGO_TIPO_VINCULACION = row1.CODIGO_TIPO_VINCULACION;
									Inscritos_tmp.VINCULACION = row1.VINCULACION;
								...
								if (Inscritos != null) {
									...
									if (Inscritos.CODIGO_TIPO_VINCULACION == null) {
										pstmt_tPostgresqlOutput_1.setNull(17,
												java.sql.Types.INTEGER);
									} else {
										pstmt_tPostgresqlOutput_1
												.setInt(17,
														Inscritos.CODIGO_TIPO_VINCULACION);
									}

									if (Inscritos.VINCULACION == null) {
										pstmt_tPostgresqlOutput_1.setNull(18,
												java.sql.Types.VARCHAR);
									} else {
										pstmt_tPostgresqlOutput_1.setString(18,
												Inscritos.VINCULACION);
									}
	...
	public void tOracleInput_2Process(
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				// linked node: tMap_4 - inputs:(row1,row2) outputs:(Inscritos)
	...
	public static void main(String[] args) {
		final evaluacion_docente_inscritos evaluacion_docente_inscritosClass = new evaluacion_docente_inscritos();

		int exitCode = evaluacion_docente_inscritosClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = evaluacion_docente_inscritos.class
					.getClassLoader().getResourceAsStream(
							"dmindicadores/evaluacion_docente_inscritos_0_1/contexts/"
									+ contextStr + ".properties");
			if (inContext == null) {
				inContext = evaluacion_docente_inscritos.class
						.getClassLoader()
						.getResourceAsStream(
		...
		if (false) {
			System.out
					.println((endUsedMemory - startUsedMemory)
							+ " bytes memory increase when running : evaluacion_docente_inscritos");
		}
...
/************************************************************************************************
 * 231922 characters generated by Talend Open Studio for Data Integration on the
 * 17 de septiembre de 2018 04:32:25 PM COT
 ************************************************************************************************/

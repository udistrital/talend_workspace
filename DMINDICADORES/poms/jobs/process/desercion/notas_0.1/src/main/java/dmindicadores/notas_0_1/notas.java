...
package dmindicadores.notas_0_1;
...
/**
 * Job: notas Purpose: <br>
 * Description:  <br>
 * @author test@talend.com
 * @version 7.1.1.20180829_1603-M3
 * @status 
 */
public class notas implements TalendJob {
	...
	private final String jobName = "notas";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass()
						...
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(notas.this, new Object[] { e,
	...
	public static class admitidosStruct implements
			routines.system.IPersistableRow<admitidosStruct> {
		final static byte[] commonByteArrayLock_DMINDICADORES_notas = new byte[0];
		static byte[] commonByteArray_DMINDICADORES_notas = new byte[0];
		...
		public Integer semestre_asignatura;

		public Integer getSemestre_asignatura() {
			return this.semestre_asignatura;
		}
		...
		public String nota_escrita;

		public String getNota_escrita() {
			return this.nota_escrita;
		}
		...
		public int hashCode() {
			if (this.hashCodeDirty) {
				...
				result = prime
						* result
						+ ((this.codigo_asignatura == null) ? 0
								: this.codigo_asignatura.hashCode());
		...
		public boolean equals(Object obj) {
			...
			if (getClass() != obj.getClass())
				return false;
			final admitidosStruct other = (admitidosStruct) obj;
			...
			if (this.codigo_asignatura == null) {
				if (other.codigo_asignatura != null)
					return false;

			} else if (!this.codigo_asignatura.equals(other.codigo_asignatura))
		...
		public void copyDataTo(admitidosStruct other) {
			...
			other.semestre_asignatura = this.semestre_asignatura;
			other.nota = this.nota;
			other.nota_escrita = this.nota_escrita;
		...
		public void copyKeysDataTo(admitidosStruct other) {
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_DMINDICADORES_notas.length) {
					if (length < 1024
							&& commonByteArray_DMINDICADORES_notas.length == 0) {
						commonByteArray_DMINDICADORES_notas = new byte[1024];
					} else {
						commonByteArray_DMINDICADORES_notas = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_DMINDICADORES_notas, 0, length);
				strReturn = new String(commonByteArray_DMINDICADORES_notas, 0,
						length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_DMINDICADORES_notas) {
				...
				try {
					...
					this.semestre_asignatura = readInteger(dis);

					this.nota = readInteger(dis);

					this.nota_escrita = readString(dis);
					...
					this.codigo_docente = readString(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeInteger(this.semestre_asignatura, dos);

				// Integer

				writeInteger(this.nota, dos);

				// String

				writeString(this.nota_escrita, dos);
				...
				writeString(this.codigo_docente, dos);
		...
		public String toString() {
			...
			sb.append(",semestre_asignatura="
					+ String.valueOf(semestre_asignatura));
			sb.append(",nota=" + String.valueOf(nota));
			sb.append(",nota_escrita=" + nota_escrita);
			...
			sb.append(",codigo_docente=" + codigo_docente);
			sb.append("]");
		 ...
		 * Compare keys
		 */
		public int compareTo(admitidosStruct other) {
			...
			returnValue = checkNullsAndCompare(this.codigo_asignatura,
					other.codigo_asignatura);
			if (returnValue != 0) {
				return returnValue;
			}
	...
	public static class row1Struct implements
		...
		public Integer NOT_ANO;

		public Integer getNOT_ANO() {
			return this.NOT_ANO;
		}

		public Integer NOT_PER;

		public Integer getNOT_PER() {
			return this.NOT_PER;
		}

		public Long NOT_EST_COD;

		public Long getNOT_EST_COD() {
			return this.NOT_EST_COD;
		}

		public Integer NOT_CRA_COD;

		public Integer getNOT_CRA_COD() {
			return this.NOT_CRA_COD;
		}

		public Integer NOT_ASI_COD;

		public Integer getNOT_ASI_COD() {
			return this.NOT_ASI_COD;
		}
		...
		public Integer NOT_SEM;

		public Integer getNOT_SEM() {
			return this.NOT_SEM;
		}

		public Integer NOT_NOTA;

		public Integer getNOT_NOTA() {
			return this.NOT_NOTA;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_DMINDICADORES_notas) {
				...
				try {
					...
					this.NOT_ANO = readInteger(dis);

					this.NOT_PER = readInteger(dis);

					length = dis.readByte();
					if (length == -1) {
						this.NOT_EST_COD = null;
					} else {
						this.NOT_EST_COD = dis.readLong();
					}

					this.NOT_CRA_COD = readInteger(dis);

					this.NOT_ASI_COD = readInteger(dis);

					this.PROYECTO = readString(dis);

					this.EST_NOMBRE = readString(dis);

					this.ASI_NOMBRE = readString(dis);

					this.NOT_SEM = readInteger(dis);

					this.NOT_NOTA = readInteger(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeInteger(this.NOT_ANO, dos);

				// Integer

				writeInteger(this.NOT_PER, dos);

				// Long

				if (this.NOT_EST_COD == null) {
					dos.writeByte(-1);
				} else {
					dos.writeByte(0);
					dos.writeLong(this.NOT_EST_COD);
				}

				// Integer

				writeInteger(this.NOT_CRA_COD, dos);

				// Integer

				writeInteger(this.NOT_ASI_COD, dos);
				...
				writeString(this.ASI_NOMBRE, dos);

				// Integer

				writeInteger(this.NOT_SEM, dos);

				// Integer

				writeInteger(this.NOT_NOTA, dos);
		...
		public String toString() {
			...
			sb.append(super.toString());
			sb.append("[");
			sb.append("NOT_ANO=" + String.valueOf(NOT_ANO));
			sb.append(",NOT_PER=" + String.valueOf(NOT_PER));
			sb.append(",NOT_EST_COD=" + String.valueOf(NOT_EST_COD));
			sb.append(",NOT_CRA_COD=" + String.valueOf(NOT_CRA_COD));
			sb.append(",NOT_ASI_COD=" + String.valueOf(NOT_ASI_COD));
			sb.append(",PROYECTO=" + PROYECTO);
			sb.append(",EST_NOMBRE=" + EST_NOMBRE);
			sb.append(",ASI_NOMBRE=" + ASI_NOMBRE);
			sb.append(",NOT_SEM=" + String.valueOf(NOT_SEM));
			sb.append(",NOT_NOTA=" + String.valueOf(NOT_NOTA));
	...
	public void tPostgresqlInput_1Process(
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				admitidosStruct admitidos = new admitidosStruct();
				...
				String dbquery_tPostgresqlInput_1 = "SELECT codigo_estudiante\n  FROM estudiantes where codigo_estudiante>7310306 order by codigo_estudiante";
				...
				try {
					...
					while (rs_tPostgresqlInput_1.next()) {
						...
						if (colQtyInRs_tPostgresqlInput_1 < 1) {
							row2.codigo_estudiante = 0;
						} else {

							if (rs_tPostgresqlInput_1.getObject(1) != null) {
								row2.codigo_estudiante = rs_tPostgresqlInput_1
										.getLong(1);
							} else {
						...
						row3.codigo_estudiante = row2.codigo_estudiante;
						...
						globalMap.put("row3.codigo_estudiante",
								row3.codigo_estudiante);
						...
						if (execStat) {
							runStat.updateStatOnConnection("admitidos", 3, 0);
						}
						...
						if (execStat) {
							runStat.updateStatOnConnection("iterate1", 1,
									"exec" + NB_ITERATE_tOracleInput_2);
							// Thread.sleep(1000);
						}
						...
						if (execStat) {
							if (resourceMap.get("inIterateVComp") == null) {
								...
								runStat.updateStatOnConnection("admitidos"
										+ iterateId, 0, 0);
						...
						if (dbschema_tPostgresqlOutput_1 == null
							...
							tableName_tPostgresqlOutput_1 = "estudiantes_notas";
						} else {
							...
							try (java.sql.Statement stmtCreate_tPostgresqlOutput_1 = conn_tPostgresqlOutput_1
								...
								stmtCreate_tPostgresqlOutput_1
										.execute("CREATE TABLE \""
												...
												+ "\"(\"anno\" INT4 ,\"semestre\" INT4 ,\"codigo_estudiante\" INT8 ,\"codigo_proyecto\" INT4 ,\"codigo_asignatura\" INT4 ,\"proyecto\" VARCHAR(100)  ,\"estudiante\" VARCHAR(50)  ,\"asignatura\" VARCHAR(100)  ,\"semestre_asignatura\" INT4 ,\"nota\" INT4 ,\"nota_escrita\" VARCHAR(100)  ,\"observaciones\" VARCHAR(50)  ,\"promedio\" FLOAT8 ,\"estado\" VARCHAR(1)  ,\"clasificacion\" VARCHAR(100)  ,\"fecha_carga\" VARCHAR ,\"grupo\" VARCHAR ,\"codigo_docente\" VARCHAR ,primary key(\"anno\",\"semestre\",\"codigo_estudiante\",\"codigo_proyecto\",\"codigo_asignatura\"))");
							}
						}
						java.sql.PreparedStatement pstmt_tPostgresqlOutput_1 = conn_tPostgresqlOutput_1
								.prepareStatement("SELECT COUNT(1) FROM \""
										+ tableName_tPostgresqlOutput_1
										+ "\" WHERE \"anno\" = ? AND \"semestre\" = ? AND \"codigo_estudiante\" = ? AND \"codigo_proyecto\" = ? AND \"codigo_asignatura\" = ?");
						resourceMap.put("pstmt_tPostgresqlOutput_1",
								pstmt_tPostgresqlOutput_1);
						String insert_tPostgresqlOutput_1 = "INSERT INTO \""
								+ tableName_tPostgresqlOutput_1
								+ "\" (\"anno\",\"semestre\",\"codigo_estudiante\",\"codigo_proyecto\",\"codigo_asignatura\",\"proyecto\",\"estudiante\",\"asignatura\",\"semestre_asignatura\",\"nota\",\"nota_escrita\",\"observaciones\",\"promedio\",\"estado\",\"clasificacion\",\"fecha_carga\",\"grupo\",\"codigo_docente\") VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
						...
						String update_tPostgresqlOutput_1 = "UPDATE \""
								+ tableName_tPostgresqlOutput_1
								+ "\" SET \"proyecto\" = ?,\"estudiante\" = ?,\"asignatura\" = ?,\"semestre_asignatura\" = ?,\"nota\" = ?,\"nota_escrita\" = ?,\"observaciones\" = ?,\"promedio\" = ?,\"estado\" = ?,\"clasificacion\" = ?,\"fecha_carga\" = ?,\"grupo\" = ?,\"codigo_docente\" = ? WHERE \"anno\" = ? AND \"semestre\" = ? AND \"codigo_estudiante\" = ? AND \"codigo_proyecto\" = ? AND \"codigo_asignatura\" = ?";
						...
						admitidosStruct admitidos_tmp = new admitidosStruct();
						// ###############################
						...
						String dbquery_tOracleInput_2 = "\n\nSELECT nota.* FROM(\nSELECT DISTINCT\nV.not_ano, \nV.not_per,\nV.not_est_cod,\nnot_cra_cod, \nV.not_asi_cod,\ncra_nombre pro"
								+ "yecto,\nest_nombre,\nS.asi_nombre,\nV.not_sem,\nV.not_nota,\nM.mon_nombre VALOR,\nCASE\nWHEN V.not_obs=0 THEN ' '\nELSE O.nob_no"
								+ "mbre\nEND obs,\nfa_promedio_nota_ano_per(V.not_est_cod,V.not_ano, V.not_per) promedio,\nnot_est_reg estado,\n(not_cea_cod||'"
								+ " '||cea_nom) CLASIFICACION,\n(lpad(cur_cra_cod,3,0)||'-'||cur_grupo)    AS grupo,\ndoc_nro_iden\nFROM acnot V\nINNER JOIN ac"
								+ "est A ON V.not_est_cod=A.est_cod\nINNER JOIN accra C ON V.not_cra_cod=C.cra_cod\nINNER JOIN acasi S ON V.not_asi_cod=S.asi"
								+ "_cod\nINNER JOIN acnotobs O ON V.not_obs=O.nob_cod\nINNER JOIN acmonto M ON V.not_nota=M.mon_cod\nINNER JOIN accursos on no"
								+ "t_gr=cur_grupo and not_asi_cod=cur_asi_cod and not_ano=cur_ape_ano and not_per=cur_ape_per\nLEFT JOIN geclasificaespac G "
								+ "ON V.not_cea_cod=G.cea_cod\nLEFT JOIN  achorarios ON hor_id_curso=cur_id\nLEFT JOIN accargas on car_hor_id=hor_id\nLEFT JOI"
								+ "N  acdocente ON doc_nro_iden=car_doc_nro\n\nUNION\nSELECT DISTINCT\nV.not_ano, \nV.not_per,\nV.not_est_cod,\nnot_cra_cod, \nV.no"
								+ "t_asi_cod,\ncra_nombre proyecto,\nest_nombre,\nS.asi_nombre,\nV.not_sem,\nV.not_nota,\nM.mon_nombre VALOR,\nCASE\nWHEN V.not_obs"
								+ "=0 THEN ' '\nELSE O.nob_nombre\nEND obs,\nfa_promedio_nota_ano_per(V.not_est_cod,V.not_ano, V.not_per) promedio,\nnot_est_re"
								+ "g estado,\n(not_cea_cod||' '||cea_nom) CLASIFICACION,\n(lpad(not_cra_cod,3,0)||'-'||cur_nro) grupo,\ncar_doc_nro_iden doc_n"
								+ "ro_iden\nFROM acnot V\nINNER JOIN acest A ON V.not_est_cod=A.est_cod\nINNER JOIN accra C ON V.not_cra_cod=C.cra_cod\nINNER J"
								+ "OIN acasi S ON V.not_asi_cod=S.asi_cod\nINNER JOIN acnotobs O ON V.not_obs=O.nob_cod\nINNER JOIN acmonto M ON V.not_nota=M"
								+ ".mon_cod\nLEFT JOIN  accursohis ON  not_gr=cur_nro and not_asi_cod=cur_asi_cod and not_ano=cur_ape_ano and not_per=cur_ap"
								+ "e_per\nLEFT JOIN accargahis  ON car_cur_asi_cod=cur_asi_cod and car_ape_ano=cur_ape_ano and \n car_ape_per=cur_ape_per and"
								+ " car_cur_nro=cur_nro and car_cra_cod=cur_cra_cod\nLEFT JOIN achorariohis on hor_asi_cod=cur_asi_cod and hor_ape_ano=cur_a"
								+ "pe_ano and \n hor_ape_per=cur_ape_per and hor_nro=cur_nro\nLEFT JOIN geclasificaespac G ON V.not_cea_cod=G.cea_cod\nLEFT JO"
								+ "IN  acdocente ON doc_nro_iden=car_doc_nro_iden\n\n) nota WHERE ESTADO <>'I' AND nota.not_est_cod="
								+ row3.codigo_estudiante
								+ "\n ORDER BY nota.not_sem, nota.not_asi_cod\n \n \n\n \n";
						...
						try {
							...
							while (rs_tOracleInput_2.next()) {
								...
								if (colQtyInRs_tOracleInput_2 < 1) {
									...
									row1.NOT_ANO = rs_tOracleInput_2.getInt(1);
								...
								if (colQtyInRs_tOracleInput_2 < 2) {
									...
									row1.NOT_PER = rs_tOracleInput_2.getInt(2);
								...
								if (colQtyInRs_tOracleInput_2 < 3) {
									row1.NOT_EST_COD = null;
								} else {

									row1.NOT_EST_COD = rs_tOracleInput_2
											.getLong(3);

									if (row1.NOT_EST_COD == 0
											&& rs_tOracleInput_2.getObject(3) == null) {

										row1.NOT_EST_COD = null;
									}
								}
								if (colQtyInRs_tOracleInput_2 < 4) {
									row1.NOT_CRA_COD = null;
								} else {

									row1.NOT_CRA_COD = rs_tOracleInput_2
											.getInt(4);

									if (row1.NOT_CRA_COD == 0
								...
								if (colQtyInRs_tOracleInput_2 < 5) {
									row1.NOT_ASI_COD = null;
								} else {

									row1.NOT_ASI_COD = rs_tOracleInput_2
											.getInt(5);

									if (row1.NOT_ASI_COD == 0
											&& rs_tOracleInput_2.getObject(5) == null) {
								...
								if (colQtyInRs_tOracleInput_2 < 7) {
									...
									row1.EST_NOMBRE = routines.system.JDBCUtil
											.getString(rs_tOracleInput_2, 7,
													true);
								}
								if (colQtyInRs_tOracleInput_2 < 8) {
									row1.ASI_NOMBRE = null;
								} else {

									row1.ASI_NOMBRE = routines.system.JDBCUtil
											.getString(rs_tOracleInput_2, 8,
													true);
								}
								if (colQtyInRs_tOracleInput_2 < 9) {
									...
									row1.NOT_SEM = rs_tOracleInput_2.getInt(9);

									if (row1.NOT_SEM == 0
											&& rs_tOracleInput_2.getObject(9) == null) {
								...
								if (colQtyInRs_tOracleInput_2 < 10) {
									...
									row1.NOT_NOTA = rs_tOracleInput_2
											.getInt(10);

									if (row1.NOT_NOTA == 0
											&& rs_tOracleInput_2.getObject(10) == null) {
								...
								if (colQtyInRs_tOracleInput_2 < 11) {
									...
									row1.VALOR = routines.system.JDBCUtil
											.getString(rs_tOracleInput_2, 11,
													true);
								}
								if (colQtyInRs_tOracleInput_2 < 12) {
									...
									row1.OBS = routines.system.JDBCUtil
											.getString(rs_tOracleInput_2, 12,
													true);
								}
								if (colQtyInRs_tOracleInput_2 < 13) {
									...
									row1.PROMEDIO = routines.system.JDBCUtil
											.getDoubleObject2(
													rs_tOracleInput_2, 13);
								}
								if (colQtyInRs_tOracleInput_2 < 14) {
									row1.ESTADO = null;
								} else {

									row1.ESTADO = routines.system.JDBCUtil
											.getString(rs_tOracleInput_2, 14,
													true);
								}
								if (colQtyInRs_tOracleInput_2 < 15) {
									row1.CLASIFICACION = null;
								} else {

									row1.CLASIFICACION = routines.system.JDBCUtil
											.getString(rs_tOracleInput_2, 15,
													true);
								}
								if (colQtyInRs_tOracleInput_2 < 16) {
									...
									row1.GRUPO = routines.system.JDBCUtil
											.getString(rs_tOracleInput_2, 16,
													true);
								}
								if (colQtyInRs_tOracleInput_2 < 17) {
									row1.DOC_NRO_IDEN = null;
								} else {

									row1.DOC_NRO_IDEN = rs_tOracleInput_2
											.getLong(17);

									if (row1.DOC_NRO_IDEN == 0
											&& rs_tOracleInput_2.getObject(17) == null) {
								...
								{ // start of Var scope
									...
									// # Output table : 'admitidos'
									admitidos_tmp.anno = row1.NOT_ANO;
									admitidos_tmp.semestre = row1.NOT_PER;
									admitidos_tmp.codigo_estudiante = row1.NOT_EST_COD;
									admitidos_tmp.codigo_proyecto = row1.NOT_CRA_COD;
									admitidos_tmp.codigo_asignatura = row1.NOT_ASI_COD;
									admitidos_tmp.proyecto = row1.PROYECTO;
									admitidos_tmp.estudiante = row1.EST_NOMBRE;
									admitidos_tmp.asignatura = row1.ASI_NOMBRE;
									admitidos_tmp.semestre_asignatura = row1.NOT_SEM;
									admitidos_tmp.nota = row1.NOT_NOTA;
									admitidos_tmp.nota_escrita = row1.VALOR;
									admitidos_tmp.observaciones = row1.OBS;
									admitidos_tmp.promedio = row1.PROMEDIO;
									admitidos_tmp.estado = row1.ESTADO;
									admitidos_tmp.clasificacion = row1.CLASIFICACION;
									admitidos_tmp.fecha_carga = TalendDate
											.getDate("DD-MM-CCYY");
									admitidos_tmp.grupo = row1.GRUPO;
									admitidos_tmp.codigo_docente = row1.DOC_NRO_IDEN != null ? row1.DOC_NRO_IDEN
											.toString() : "NO_REGISTRA";
									admitidos = admitidos_tmp;
									// ###############################
								...
								if (admitidos != null) {
									...
									if (execStat) {
										runStat.updateStatOnConnection(
												"admitidos" + iterateId, 1, 1);
									}

									whetherReject_tPostgresqlOutput_1 = false;
									if (admitidos.anno == null) {
									...
									if (admitidos.codigo_estudiante == null) {
										pstmt_tPostgresqlOutput_1.setNull(3,
												java.sql.Types.INTEGER);
									} else {
										pstmt_tPostgresqlOutput_1.setLong(3,
												admitidos.codigo_estudiante);
									}

									if (admitidos.codigo_proyecto == null) {
										pstmt_tPostgresqlOutput_1.setNull(4,
												java.sql.Types.INTEGER);
									} else {
										pstmt_tPostgresqlOutput_1.setInt(4,
												admitidos.codigo_proyecto);
									}

									if (admitidos.codigo_asignatura == null) {
										pstmt_tPostgresqlOutput_1.setNull(5,
												java.sql.Types.INTEGER);
									} else {
										pstmt_tPostgresqlOutput_1.setInt(5,
												admitidos.codigo_asignatura);
									}

									int checkCount_tPostgresqlOutput_1 = -1;
									try (java.sql.ResultSet rs_tPostgresqlOutput_1 = pstmt_tPostgresqlOutput_1
											.executeQuery()) {
									...
									if (checkCount_tPostgresqlOutput_1 > 0) {
										...
										if (admitidos.estudiante == null) {
											...
											pstmtUpdate_tPostgresqlOutput_1
													.setString(
															2,
															admitidos.estudiante);
										}

										if (admitidos.asignatura == null) {
											pstmtUpdate_tPostgresqlOutput_1
													.setNull(
															3,
															java.sql.Types.VARCHAR);
											...
											pstmtUpdate_tPostgresqlOutput_1
													.setString(
															3,
															admitidos.asignatura);
										}

										if (admitidos.semestre_asignatura == null) {
											pstmtUpdate_tPostgresqlOutput_1
													.setNull(
															4,
															java.sql.Types.INTEGER);
											...
											pstmtUpdate_tPostgresqlOutput_1
													.setInt(4,
															admitidos.semestre_asignatura);
										}

										if (admitidos.nota == null) {
											pstmtUpdate_tPostgresqlOutput_1
													.setNull(
															5,
															java.sql.Types.INTEGER);
											...
											pstmtUpdate_tPostgresqlOutput_1
													.setInt(5, admitidos.nota);
										}

										if (admitidos.nota_escrita == null) {
											pstmtUpdate_tPostgresqlOutput_1
													.setNull(
															6,
															java.sql.Types.VARCHAR);
											...
											pstmtUpdate_tPostgresqlOutput_1
													.setString(
															6,
															admitidos.nota_escrita);
										}

										if (admitidos.observaciones == null) {
											pstmtUpdate_tPostgresqlOutput_1
													.setNull(
															7,
															java.sql.Types.VARCHAR);
											...
											pstmtUpdate_tPostgresqlOutput_1
													.setString(
															7,
															admitidos.observaciones);
										}

										if (admitidos.promedio == null) {
											pstmtUpdate_tPostgresqlOutput_1
													.setNull(
															8,
										...
										if (admitidos.clasificacion == null) {
											pstmtUpdate_tPostgresqlOutput_1
													.setNull(
															10,
															java.sql.Types.VARCHAR);
											...
											pstmtUpdate_tPostgresqlOutput_1
													.setString(
															10,
															admitidos.clasificacion);
										}

										if (admitidos.fecha_carga == null) {
											pstmtUpdate_tPostgresqlOutput_1
													.setNull(
															11,
															java.sql.Types.VARCHAR);
											...
											pstmtUpdate_tPostgresqlOutput_1
													.setString(
															11,
															admitidos.fecha_carga);
										}

										if (admitidos.grupo == null) {
											pstmtUpdate_tPostgresqlOutput_1
													.setNull(
															12,
															java.sql.Types.VARCHAR);
										...
										if (admitidos.codigo_docente == null) {
											pstmtUpdate_tPostgresqlOutput_1
													.setNull(
															13,
															java.sql.Types.VARCHAR);
											...
											pstmtUpdate_tPostgresqlOutput_1
													.setString(
															13,
															admitidos.codigo_docente);
										}

										if (admitidos.anno == null) {
											pstmtUpdate_tPostgresqlOutput_1
													.setNull(
															14 + count_tPostgresqlOutput_1,
															java.sql.Types.INTEGER);
										} else {
											pstmtUpdate_tPostgresqlOutput_1
													.setInt(14 + count_tPostgresqlOutput_1,
															admitidos.anno);
										}
										...
										if (admitidos.semestre == null) {
											...
											pstmtUpdate_tPostgresqlOutput_1
													.setInt(15 + count_tPostgresqlOutput_1,
															admitidos.semestre);
										}
										...
										if (admitidos.codigo_proyecto == null) {
											pstmtUpdate_tPostgresqlOutput_1
													.setNull(
															17 + count_tPostgresqlOutput_1,
															java.sql.Types.INTEGER);
										} else {
											pstmtUpdate_tPostgresqlOutput_1
													.setInt(17 + count_tPostgresqlOutput_1,
															admitidos.codigo_proyecto);
										}
										...
										if (admitidos.codigo_asignatura == null) {
											pstmtUpdate_tPostgresqlOutput_1
													.setNull(
															18 + count_tPostgresqlOutput_1,
															java.sql.Types.INTEGER);
										} else {
											pstmtUpdate_tPostgresqlOutput_1
													.setInt(18 + count_tPostgresqlOutput_1,
															admitidos.codigo_asignatura);
										}
	...
	public static void main(String[] args) {
		final notas notasClass = new notas();

		int exitCode = notasClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = notas.class.getClassLoader()
					.getResourceAsStream(
							"dmindicadores/notas_0_1/contexts/" + contextStr
									+ ".properties");
			if (inContext == null) {
				inContext = notas.class.getClassLoader().getResourceAsStream(
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory)
					+ " bytes memory increase when running : notas");
		}
...
/************************************************************************************************
 * 211345 characters generated by Talend Open Studio for Data Integration on the
 * 25 de septiembre de 2018 04:54:06 PM COT
 ************************************************************************************************/

...
public class TWriteXMLFieldOutBeginJava
{
  ...
  protected final String TEXT_63 = " = new java.util.HashMap<String,String>();";
  protected final String TEXT_64 = NL + "java.util.Map<String,String> arraysValueMap_";
  protected final String TEXT_65 = " = new java.util.HashMap<String,String>();";
  protected final String TEXT_66 = NL + NL + "class NestXMLTool_";
  protected final String TEXT_67 = "{" + NL + "\tpublic void parseAndAdd(org.dom4j.Element nestRoot, String value){" + NL + "\t\ttry {" + NL + "            org.dom4j.Document doc4Str = org.dom4j.DocumentHelper.parseText(\"<root>\"+ value + \"</root>\");" + NL + "    \t\tnestRoot.setContent(doc4Str.getRootElement().content());" + NL + "    \t} catch (java.lang.Exception e) {" + NL + "    \t\te.printStackTrace();" + NL + "    \t\tnestRoot.setText(value);" + NL + "        }" + NL + "\t}" + NL + "\t" + NL + "\tpublic void setText(org.dom4j.Element element, String value){" + NL + "\t\tif (value.startsWith(\"<![CDATA[\") && value.endsWith(\"]]>\")) {" + NL + "\t\t\tString text = value.substring(9, value.length()-3);" + NL + "\t\t\telement.addCDATA(text);" + NL + "\t\t}else{" + NL + "\t\t\telement.setText(value);" + NL + "\t\t}" + NL + "\t}" + NL + "\t" + NL + "\tpublic void replaceDefaultNameSpace(org.dom4j.Element nestRoot){" + NL + "\t\tif (nestRoot!=null) {" + NL + "\t\t\tfor (org.dom4j.Element tmp: (java.util.List<org.dom4j.Element>) nestRoot.elements()) {" + NL + "        \t\tif ((\"\").equals(tmp.getQName().getNamespace().getURI()) && (\"\").equals(tmp.getQName().getNamespace().getPrefix())){" + NL + "        \t\t\ttmp.setQName(org.dom4j.DocumentHelper.createQName(tmp.getName(), nestRoot.getQName().getNamespace()));" + NL + "\t        \t}" + NL + "    \t    \treplaceDefaultNameSpace(tmp);" + NL + "       \t\t}" + NL + "       \t}" + NL + "\t}" + NL + "\t" + NL + "\tpublic void removeEmptyElement(org.dom4j.Element root){" + NL + "\t\tif (root!=null) {" + NL + "\t\t\tfor (org.dom4j.Element tmp: (java.util.List<org.dom4j.Element>) root.elements()) {" + NL + "\t\t\t\tremoveEmptyElement(tmp);" + NL + "\t\t\t}" + NL + "\t\t\tif (root.content().size() == 0 " + NL + "    \t\t\t&& root.attributes().size() == 0 " + NL + "    \t\t\t&& root.declaredNamespaces().size() == 0) {" + NL + "    \t\t\tif(root.getParent()!=null){" + NL + "                \troot.getParent().remove(root);" + NL + "                }" + NL + "            }" + NL + "\t\t}" + NL + "\t}" + NL + "\t" + NL + "\tpublic String objectToString(Object value){" + NL + "\t\tif(value.getClass().isArray()){" + NL + "\t\t\tStringBuilder sb = new StringBuilder();" + NL + "" + NL + "\t\t\tint length = java.lang.reflect.Array.getLength(value);" + NL + "\t\t\tfor (int i = 0; i < length; i++) {" + NL + "\t\t\t\tObject obj = java.lang.reflect.Array.get(value, i);" + NL + "\t\t\t\tsb.append(\"<element>\");" + NL + "\t\t\t\tsb.append(obj);" + NL + "\t\t\t\tsb.append(\"</element>\");" + NL + "\t\t\t}" + NL + "\t\t\treturn sb.toString();" + NL + "\t\t}else{" + NL + "\t\t\treturn value.toString();" + NL + "\t\t}" + NL + "\t}" + NL + "}" + NL + "NestXMLTool_";
  protected final String TEXT_68 = " nestXMLTool_";
  protected final String TEXT_69 = " = new NestXMLTool_";
  protected final String TEXT_70 = "();" + NL;
  protected final String TEXT_71 = NL;
  protected final String TEXT_72 = "Struct  rowStructOutput_";
  protected final String TEXT_73 = " = new ";
  protected final String TEXT_74 = "Struct();" + NL + "// sort group root element for judgement of group" + NL + "java.util.List<org.dom4j.Element> groupElementList_";
  protected final String TEXT_75 = " = new java.util.ArrayList<org.dom4j.Element>();" + NL + "org.dom4j.Element root4Group_";
  protected final String TEXT_76 = " = null;" + NL + "org.dom4j.Document doc_";
  protected final String TEXT_77 = "  = org.dom4j.DocumentHelper.createDocument();";
  protected final String TEXT_78 = NL + "org.dom4j.io.OutputFormat format_";
  protected final String TEXT_79 = " = org.dom4j.io.OutputFormat.createCompactFormat();" + NL + "format_";
  protected final String TEXT_80 = ".setNewLineAfterDeclaration(false);";
  protected final String TEXT_81 = NL + "org.dom4j.io.OutputFormat format_";
  protected final String TEXT_82 = " = org.dom4j.io.OutputFormat.createPrettyPrint();";
  protected final String TEXT_83 = NL + "format_";
  protected final String TEXT_84 = ".setTrimText(false);" + NL + "format_";
  protected final String TEXT_85 = ".setEncoding(";
  protected final String TEXT_86 = ");";
  protected final String TEXT_87 = "format_";
  protected final String TEXT_88 = ".setExpandEmptyElements(true);";
  protected final String TEXT_89 = NL + "int[] orders_";
  protected final String TEXT_90 = " = new int[";
  protected final String TEXT_91 = "];";
  protected final String TEXT_92 = NL + "java.util.List<String> endTabStrs_";
  protected final String TEXT_93 = " = new java.util.ArrayList<String>();" + NL + "java.util.List<String> startTabStrs_";
  protected final String TEXT_94 = " = new java.util.ArrayList<String>();";
  protected final String TEXT_95 = NL + "endTabStrs_";
  protected final String TEXT_96 = ".add(\"";
  protected final String TEXT_97 = "</";
  protected final String TEXT_98 = ">\");" + NL + "startTabStrs_";
  protected final String TEXT_99 = ".add(\"";
  protected final String TEXT_100 = "<";
  protected final String TEXT_101 = ">\");";
  protected final String TEXT_102 = NL + "int preUnNullMaxIndex_";
  protected final String TEXT_103 = " = -1;" + NL + "int preNewTabIndex_";
  protected final String TEXT_104 = " = -1;" + NL + "String[] startTabs_";
  protected final String TEXT_105 = " = new String[endTabStrs_";
  protected final String TEXT_106 = ".size()];" + NL + "String[] endTabs_";
  protected final String TEXT_107 = " = new String[endTabStrs_";
  protected final String TEXT_108 = ".size()];" + NL + "java.io.StringWriter strWriter_";
  protected final String TEXT_109 = " = new java.io.StringWriter();" + NL + "java.io.BufferedWriter out_";
  protected final String TEXT_110 = " = new java.io.BufferedWriter(strWriter_";
  protected final String TEXT_111 = ");";
  protected final String TEXT_112 = NL + "out_";
  protected final String TEXT_113 = ".write(\"<?xml version=\\\"1.0\\\" encoding=\\\"\"+";
  protected final String TEXT_114 = "+\"\\\"?>\");" + NL + "out_";
